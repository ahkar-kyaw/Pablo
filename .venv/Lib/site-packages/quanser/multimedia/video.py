...
ffi.cdef("""
    ...
    t_error video3d_stream_get_depth_scale(t_video3d_stream stream, t_image_data_type data_type, t_single* depth_scale);
...
media_lib = ffi.dlopen(add_common_path("quanser_media"))
...
class Video3DStream:
    ...
    def get_depth_scale(self, data_type):
        """Gets the scale factor from depth expressed in the given data type to metres.

        Parameters
        ----------
        data_type : ImageDataType
            The data type of the depth image for which to get the scale factor to metres.
        ...
        ...                              ImageFormat.ROW_MAJOR_GREYSCALE, ImageDataType.UINT16)
        ...
        >>> depth_scale = depth_stream.get_depth_scale(ImageDataType.UINT16)
        ...
        >>> video3d.stop_streaming()
        >>> depth_stream.close()
        >>> video3d.close()
        ...
        """
        depth_scale = ffi.new("t_single *")
        result = media_lib.video3d_stream_get_depth_scale(self.stream if self._stream is not None else ffi.NULL, data_type, depth_scale)
        if result < 0:
            raise MediaError(result)
